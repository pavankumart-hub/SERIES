...
ticker = st.sidebar.text_input("Enter Stock Ticker (e.g., RELIANCE, AAPL)", "RELIANCE").upper()
...
if run_analysis:
    # Handle Indian tickers automatically
    if not any(suffix in ticker for suffix in [".NS", ".BO", ".NSE", ".BSE", ".AX", ".L"]):
        ticker += ".NS"
    ...
    if data.empty:
        st.error("No data found for this ticker or date range. Try adding '.NS' for Indian stocks.")
        st.stop()
    ...
    # Ensure same shape
    forecast_mean = np.array(forecast_mean).flatten()
    conf_int = conf_int.to_numpy()
