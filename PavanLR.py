...
if run_analysis:
    ...
    try:
        ...
        forecast = best_model_fitted.get_forecast(steps=forecast_steps)
        forecast_mean = forecast.predicted_mean.values
        conf_int = forecast.conf_int().values

        # Debug information
        st.write(f"🔍 Debug Info:")
        st.write(f"- Forecast mean length: {len(forecast_mean)}")
        st.write(f"- Confidence interval shape: {conf_int.shape}")

        # Create forecast dates
        last_date = series.index[-1]
        forecast_dates = [last_date + timedelta(days=i) for i in range(1, forecast_steps + 1)]

        # Ensure we have the right number of forecast dates
        forecast_dates = forecast_dates[:len(forecast_mean)]

        # Reconstruct to original scale based on stationarity type
        if stationary_type == "trend_stationary":
        ...
        # Final dimension check and alignment
        min_length = min(len(forecast_dates), len(forecast_mean_orig), len(conf_int_orig))

        forecast_dates_final = forecast_dates[:min_length]
        forecast_mean_final = forecast_mean_orig[:min_length]
        conf_int_final = conf_int_orig[:min_length]

        st.write(f"🔍 Final dimensions:")
        st.write(f"- Dates: {len(forecast_dates_final)}")
        st.write(f"- Forecast: {len(forecast_mean_final)}")
        st.write(f"- CI: {len(conf_int_final)}")
        ...
        # Plot forecast - ensure same dimensions
